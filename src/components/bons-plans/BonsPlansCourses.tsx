...
import { ShoppingCart, ChevronDown, ChevronUp, MapPin, TrendingDown, Flame } from "lucide-react";
    ...
    <div className="rounded-3xl border border-border bg-card p-5">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-bold text-foreground">Le panier malin 🛒</h3>
          <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
            <MapPin className="h-3 w-3" />
            <span>Les meilleurs spots à {displayCity} · sans te ruiner</span>
          </div>
        </div>
      </div>
        ...
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-success/20 bg-success/5 px-3 py-2">
          ...
          <p className="text-xs text-foreground">
            ...
            <span className="text-muted-foreground">
              c'est le bon plan du moment ({cheapestStore[1].toFixed(2)}€ le panier)
            </span>
          </p>
        </div>
      )}
      ...
      {products.length > 3 && (
        ...
        >
          {expanded ? (
            ...
            <>Voir {products.length - 3} autres bons plans <ChevronDown className="h-3 w-3" /></>
          )}
        </button>
      )}

      <p className="mt-3 text-[10px] text-muted-foreground text-center leading-relaxed">
        💡 Sélection maison · prix donnés à titre indicatif, ça peut bouger en magasin.
      </p>
    </div>
  );
};

export default BonsPlansCourses;